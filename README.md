...
### v2.9.3
- 📝 增强#成就查漏指令的日志记录功能，添加详细的执行过程日志
- 🔍 添加更多调试信息，包括正则匹配结果、文件检查状态等
- 🔧 优化错误处理，提供更详细的错误信息日志

### v2.9.2
- 🐛 修复compareAchievements函数中的逻辑错误，确保#成就查漏指令能正确筛选目标类目
- 🔄 优化条件判断逻辑，使其更清晰易懂

### v2.9.1
- 🐛 重新实现#成就查漏[类目名]指令的正则表达式匹配逻辑，使用更简单直接的方式确保所有格式（#成就查漏+类目名、#成就查漏 类目名、#成就查漏类目名）都能正确识别

### v2.9.0
- 🐛 修复#成就查漏[类目名]指令的正则表达式匹配问题，确保所有格式（#成就查漏+类目名、#成就查漏 类目名、#成就查漏类目名）都能正确识别

### v2.8.9
- ✨ 将#成就查漏[类目名]指令添加到#成就帮助生成的图片中，方便用户查看
- 🐛 修复成就查漏指令解析，支持多种格式：#成就查漏+类目名、#成就查漏 类目名、#成就查漏类目名

### v2.8.8
- ✨ 优化成就查漏功能，现在"天地万象"类目的未完成成就会优先显示
- ✨ 新增#成就查漏+[类目名]指令，支持查询指定类目的未完成成就

### v2.8.7
- 修复ACM更新功能在执行时发送单条反馈信息的问题，现在所有反馈信息都会合并为一条消息发送

### v2.8.6
- 🐛 修复#ACM更新命令多次发送合并消息的问题，现在只发送一次合并反馈
- 🔄 优化反馈消息处理逻辑，确保用户体验一致性

### v2.8.5
- 🐛 修复#ACM更新命令在#成就帮助图片中的显示问题
- 🔄 优化命令排序，确保更新命令正确显示
- ✨ 改进ACM更新功能，使用直接URL拉取更新，避免remote配置问题
- 📝 优化错误处理流程，确保反馈信息正确合并转发给用户
- 🔧 修复代码结构问题，移除重复的try-catch-finally块

### v2.8.4
- 🐛 修复Gitee备用仓库功能，确保在GitLab不可访问时能够正确切换到Gitee
- ✨ 优化错误处理逻辑，提供更清晰的网络连接状态反馈

### v2.8.3
- ✨ #ACM更新 命令现已显示在#成就帮助中
- 🔄 优化更新机制，添加Gitee备用仓库支持
- 🐛 修复可能的内存泄漏警告

### v2.8.2
- ✨ 新增 #ACM更新 指令，支持一键检查和更新插件（仅管理员可用）

### v2.8.1
- 🐛 修复比对椰羊数据时不排除timestamp为0的成就
